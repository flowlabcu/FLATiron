...
where :math:`\mathcal{S}` is the stabilization term for advection dominated problem **CITE**. There are three types of :math:`\tau` that the code provides. These are the `shakib`, `su`, `codina` type. For given a cell diameter :math:`h`, a cell Peclet number :math:`Pe_h=\frac{\lvert \textbf{u} \rvert h}{2D}` **CITE Donea**.
