...
class InputObject:
    """
    Class for reading input file for FLATiron apps 
    Parameters
    ----------
    input_file : str
        Path to input file
    ...
    def __call__(self, key, required=False, warn=True):
        ...
        try:
            ...
            if required:
                raise Exception("\033[91m[ERROR]\033[0m MISSING REQUIRED INPUT: \'%s\'" % key)
            else:
                if warn:
                    print("\033[91m[WARNING]\033[0m KEY/VALUE PAIR NOT FOUND: \'%s\'" % key )
        return output
